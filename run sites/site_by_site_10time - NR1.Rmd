...
Selected <- sample.int(nrow(obs_4[[2]][[10]]),floor(365*0.333*df_drivers2[10,]$params_siml[[1]]$nyeartrend))
obs_4[[2]][[10]][-Selected,]$le <- NA


pars <- calib_sofun(
    drivers = df_drivers2[10,],  
    obs = obs_4,
    settings = settings
  )
c_NR<-tibble()
c_NR<- tibble(
  sitename = sitenames[[10]],
  exp_et   = pars$par[[1]],
  beta_et  = pars$par[[2]],
  rwsc     = pars$par[[3]]
)
