...
Selected <- sample.int(nrow(obs_4[[2]][[12]]),floor(365*0.333*df_drivers2[12,]$params_siml[[1]]$nyeartrend))
obs_4[[2]][[12]][-Selected,]$le <- NA


pars <- calib_sofun(
    drivers = df_drivers2[12,],  
    obs = obs_4,
    settings = settings
  )
c_Ton<-tibble()
c_Ton<- tibble(
  sitename = sitenames[12],
  exp_et   = pars$par[[1]],
  beta_et  = pars$par[[2]],
  rwsc     = pars$par[[3]]
)
