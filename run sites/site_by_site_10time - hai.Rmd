...
Selected <- sample.int(nrow(obs_4[[2]][[3]]),floor(365*0.5*df_drivers2[3,]$params_siml[[1]]$nyeartrend))
obs_4[[2]][[3]][-Selected,]$le <- NA


pars <- calib_sofun(
    drivers = df_drivers2[3,],  
    obs = obs_4,
    settings = settings
  )
c_hai<-tibble()
c_hai <- tibble(
  sitename = sitenames[3],
  exp_et   = pars$par[[1]],
  beta_et  = pars$par[[2]],
  rwsc     = pars$par[[3]]
)
