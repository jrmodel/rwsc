...
Selected <- sample.int(nrow(obs_4[[2]][[6]]),floor(365*0.333*df_drivers2[6,]$params_siml[[1]]$nyeartrend))
obs_4[[2]][[6]][-Selected,]$le <- NA


pars <- calib_sofun(
    drivers = df_drivers2[6,],  
    obs = obs_4,
    settings = settings
  )
c_Pue<-tibble()
c_Pue<- tibble(
  sitename = sitenames[[6]],
  exp_et   = pars$par[[1]],
  beta_et  = pars$par[[2]],
  rwsc     = pars$par[[3]]
)
