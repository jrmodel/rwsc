...
Selected <- sample.int(nrow(obs_4[[2]][[9]]),floor(365*0.333*df_drivers2[9,]$params_siml[[1]]$nyeartrend))
obs_4[[2]][[9]][-Selected,]$le <- NA


pars <- calib_sofun(
    drivers = df_drivers2[9,],  
    obs = obs_4,
    settings = settings
  )
c_ME<-tibble()
c_ME<- tibble(
  sitename = sitenames[[9]],
  exp_et   = pars$par[[1]],
  beta_et  = pars$par[[2]],
  rwsc     = pars$par[[3]]
)
...
for ( i in 1:9){
  obs_4 <- obs_3
Selected <- sample.int(nrow(obs_4[[2]][[9]]),floor(365*0.5*df_drivers2[9,]$params_siml[[1]]$nyeartrend))
obs_4[[2]][[9]][-Selected,]$le <- NA
