...
Selected <- sample.int(nrow(obs_4[[2]][[7]]),floor(365*0.333*df_drivers2[7,]$params_siml[[1]]$nyeartrend))
obs_4[[2]][[7]][-Selected,]$le <- NA


pars <- calib_sofun(
    drivers = df_drivers2[7,],  
    obs = obs_4,
    settings = settings
  )
c_Guy<-tibble()
c_Guy<- tibble(
  sitename = sitenames[[7]],
  exp_et   = pars$par[[1]],
  beta_et  = pars$par[[2]],
  rwsc     = pars$par[[3]]
)
