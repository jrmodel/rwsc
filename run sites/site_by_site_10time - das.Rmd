...
Selected <- sample.int(nrow(obs_4[[2]][[2]]),floor(365*0.5*df_drivers2[2,]$params_siml[[1]]$nyeartrend))
obs_4[[2]][[2]][-Selected,]$le <- NA


pars <- calib_sofun(
    drivers = df_drivers2[2,],  
    obs = obs_4,
    settings = settings
  )
c_das<-tibble()
c_das <- tibble(
  sitename = sitenames[[2]],
  exp_et   = pars$par[[1]],
  beta_et  = pars$par[[2]],
  rwsc     = pars$par[[3]]
)
...
for ( i in 1:9){
  obs_4 <- obs_3
Selected <- sample.int(nrow(obs_4[[2]][[2]]),floor(365*0.333*df_drivers2[2,]$params_siml[[1]]$nyeartrend))
obs_4[[2]][[2]][-Selected,]$le <- NA
