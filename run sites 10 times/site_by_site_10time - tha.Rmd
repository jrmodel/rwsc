...
Selected <- sample.int(nrow(obs_4[[2]][[4]]),floor(365*0.5*df_drivers2[4,]$params_siml[[1]]$nyeartrend))
obs_4[[2]][[4]][-Selected,]$le <- NA


pars <- calib_sofun(
    drivers = df_drivers2[4,],  
    obs = obs_4,
    settings = settings
  )
c_tha<-tibble()
c_tha<- tibble(
  sitename = sitenames[[4]],
  exp_et   = pars$par[[1]],
  beta_et  = pars$par[[2]],
  rwsc     = pars$par[[3]]
)
