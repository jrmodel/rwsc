...
Selected <- sample.int(nrow(obs_4[[2]][[5]]),floor(365*0.5*df_drivers2[5,]$params_siml[[1]]$nyeartrend))
obs_4[[2]][[5]][-Selected,]$le <- NA


pars <- calib_sofun(
    drivers = df_drivers2[5,],  
    obs = obs_4,
    settings = settings
  )
c_Lbr<-tibble()
c_Lbr<- tibble(
  sitename = sitenames[[5]],
  exp_et   = pars$par[[1]],
  beta_et  = pars$par[[2]],
  rwsc     = pars$par[[3]]
)
