...
Selected <- sample.int(nrow(obs_4[[2]][[8]]),floor(365*0.333*df_drivers2[8,]$params_siml[[1]]$nyeartrend))
obs_4[[2]][[8]][-Selected,]$le <- NA


pars <- calib_sofun(
    drivers = df_drivers2[8,],  
    obs = obs_4,
    settings = settings
  )
c_Loo<-tibble()
c_Loo<- tibble(
  sitename = sitenames[[8]],
  exp_et   = pars$par[[1]],
  beta_et  = pars$par[[2]],
  rwsc     = pars$par[[3]]
)
